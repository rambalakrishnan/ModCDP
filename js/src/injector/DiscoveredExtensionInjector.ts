...
export class DiscoveredExtensionInjector extends ExtensionInjector {
  async inject() {
    ...
    if (this.options.injector_trust_service_worker_target) {
      const waited = await this.waitForReadyServiceWorker(this.options.injector_service_worker_probe_timeout_ms ?? 10_000, {
        matched_only: true,
      });
    ...
    const waited = await this.waitForReadyServiceWorker(this.options.injector_service_worker_ready_timeout_ms ?? 60_000, {
      matched_only: this.options.injector_trust_service_worker_target,
    });
    if (waited) return { ...waited, source: "discovered" };
    throw new Error(
